...
*   V1.0b 22-Sep-95, J Love	INITIALRECS/ALLOCATERECS in `AllocateRecordsXP'.
...
static Logical SetBlocking PROTOARGs((char *field, struct VarStructXP *Var));
static Logical SetMonotonic PROTOARGs((char *field, struct VarStructXP *Var));
static Logical SetSparseness PROTOARGs((char *field, struct VarStructXP *Var));
static Logical SetCompression PROTOARGs((char *field, struct VarStructXP *Var));
...
static Logical AllocateRecordsXP PROTOARGs((
...
static CDFstatus BuildWalkScreen PROTOARGs((
  ...
  char *lineS[], int lineNs[], int colS[], int lenS[], struct VarStructXP *Vars[]
));
...
static Logical PromptForVariableSearch PROTOARGs((
  struct VarStructXP *Var, int *searchType, void **searchValue
));
static CDFstatus SearchForVariableValue PROTOARGs((
  struct VarStructXP *Var, long numDims, long dimSizes[], long *recordAt,
	     ...
	     if (STRINGdataType(Item->Var->dataType))
	       ...
	       char temp[MAX_nonSTRING_VALUE_LEN+1];
	           ...
	           EncodeValueFormat (Item->Var->dataType,
				      Item->Var->min, temp, Item->Var->format,
				      -13, MAX_nonSTRING_VALUE_LEN, style,
				      (size_t) sizeof(temp));
	   ...
	   if (Item->Var->max != NULL) {
	     if (STRINGdataType(Item->Var->dataType))
	       ...
	       char temp[MAX_nonSTRING_VALUE_LEN+1];
	           ...
	           EncodeValueFormat (Item->Var->dataType,
				      Item->Var->max, temp, Item->Var->format,
				      -13, MAX_nonSTRING_VALUE_LEN, style,
				      (size_t) sizeof(temp));
               } else if (DOUBLEdataType(Item->Var->dataType)) {
	           ...
	           EncodeValueFormat (CDF_REAL8, Item->Var->max, temp, "%.13e",
				      0, MAX_nonSTRING_VALUE_LEN, style,
				      (size_t) sizeof(temp));
               } else if (Item->Var->dataType == CDF_EPOCH16) {
                 if (!isnan(*(double *)Item->Var->max) &&
...
void RestoreSettings () {
  ...
  while ((linePtr = fgets(line,MAX_SETTINGS_LEN+1+1,fp)) != NULL) {
	...
	*commaPtr = NUL;
	if (!SetOutput(fieldPtr,Item)) break;
        if (outputVars == 2L && Item->outputSetting) {
          vars[numVars] = (char *) cdf_AllocateMemory(
						(size_t)CDF_VAR_NAME_LEN256+1,
                                                FatalError);
	...
	**********************************************************************/
	...
	if (*fieldPtr == SETTINGS_DELIM) {
	  ...
	  *commaPtr = NUL;
	  ...
	  Item->Var->format = (char *) cdf_AllocateMemory (
						(size_t)strlen(fieldPtr),
						FatalError);
...
long *lastRec;
{
  ...
  CDFstatus status; struct ItemStruct *Item; struct HyperStruct hyperSt;
  ...
  ****************************************************************************/
  if (filterCount == 0) {
    ...
    if (!AllocateRecordsXP(outID,scalarHead,hyperHead)) return FALSE;
    return TRUE;
  }
  /****************************************************************************
  ...
  ****************************************************************************/
  InitHyperParms (&hyperSt, &groups, 0L, &nHypers, &nValues);
  for (hyperN = 0; hyperN < nHypers; hyperN++) {
	...
	if (Item->filter) {
	  status = HYPERget (inID, Item->Var->varN, Item->Var->zVar,
			     *firstRec + hyperSt.recNumber, hyperSt.recCount,
	...
	* filters.
	**********************************************************************/
	firstX = FindFirstRecord (recX, scalarHead, TRUE, hyperSt.recCount);
	if (firstX == NO_RECORD) break;
	lastX = FindLastRecord (firstX, scalarHead, TRUE, hyperSt.recCount);
	/**********************************************************************
	* Calculate the absolute record number of this range and set the new
	* first/last records accordingly.
	**********************************************************************/
	recF = *firstRec + hyperSt.recNumber + firstX;
	recL = *firstRec + hyperSt.recNumber + lastX;
	...
	**********************************************************************/
	recX = lastX + 1;
	if (recX == hyperSt.recCount) break;
     }
     /*************************************************************************
     * Increment to next hyper.
     *************************************************************************/
     IncrHyperParms (&hyperSt, &groups, 0L, ROWmajor(inMajority), &nValues);
  }
  /****************************************************************************
  * Free memory used.
  ****************************************************************************/
...
static Logical AllocateRecordsXP (outID, scalarHead, hyperHead)
CDFid outID;
struct ItemStruct *scalarHead;
struct ItemStruct *hyperHead;
{
  ...
  ****************************************************************************/
  for (Item = scalarHead; Item != NULL; Item = Item->nextScalar) {
     if (Item->output && Item->Var->oRecords > 0) {
       struct VarStructXP *Var = Item->Var;
...
Logical ToWalk () {
  ...
  struct VarStructXP *Vars[MAXwalkingVARs];
  ...
  static int blenS[1+CDF_MAX_DIMS+MAXwalkingVARs];
  AOSs15 (lineS, BLANKs78, BLANKs78, BLANKs78, BLANKs78, BLANKs78,
	         BLANKs78, BLANKs78, BLANKs78, BLANKs78, BLANKs78,
	         BLANKs78, BLANKs78, BLANKs78, BLANKs78, BLANKs78)
  ...
  static struct ItemWindowStruct IW = {
    ...
    lenS, blenS, 2 + MAXwalkingVARs, 0, NULL, exitChars, REFRESHkey_FSI, FALSE,
    NUL, NUL
  };
  static char keyDefsBlank[] = "\n\n";
...
void **searchValue;
{
  ...
  ****************************************************************************/
  ...
  if (FLOAT4dataType(Var->dataType)) {
    if (!isnan((double)*(float *)Var->value) &&
      ...
      EncodeValuesFormat (Var->dataType, Var->numElems, Var->value, value,
		          ...
		          (size_t) sizeof(value));
  } else if (DOUBLEdataType(Var->dataType)) {
...
CDFstatus GetVirtualVariableFunc(Logical zVar, char *varName, char *funcName) {
  ...
  status = CDFlib(SELECT_, ATTR_NAME_, "FUNCT",
                           BOO(zVar,zENTRY_NAME_,rENTRY_NAME_), varName,
                  GET_, BOO(zVar,zENTRY_NUMELEMS_,rENTRY_NUMELEMS_), &numElms,
                        BOO(zVar,zENTRY_DATATYPE_,rENTRY_DATATYPE_), &dataType,
                  NULL_);
  if (status != CDF_OK) {
    funcName = NULL;
    return status;
  }
  tmp = malloc (numElms + 1);
  status = CDFlib(SELECT_, ATTR_NAME_, "FUNCT",
                           BOO(zVar,zENTRY_NAME_,rENTRY_NAME_), varName,
                  GET_, BOO(zVar,zENTRY_DATA_,rENTRY_DATA_), tmp,
                  NULL_);
  *(tmp+(int)numElms) = (char) 0;
  strcpy (funcName, tmp);
  free (tmp);
  tmp = NULL;
  return status;
...
void **out;
{
  ...
  if (!IsaVirtualVariable(zVar, varName)) {
    ...
    char *tcomp0, *tcomp1, *utf8;
    status = CDFlib(SELECT_, ATTR_NAME_, "COMPONENT_0",
                             zENTRY_NAME_, varName,
                    GET_, zENTRY_NUMELEMS_, &numElms1,
                          zENTRY_DATATYPE_, &dataType,
                    SELECT_, ATTR_NAME_, "COMPONENT_1",
                             zENTRY_NAME_, varName,
                    GET_, zENTRY_NUMELEMS_, &numElms2,
                    NULL_);
    if (status != CDF_OK) {
      return status;
    } 
    tcomp0 = malloc(numElms1 + 1);
    tcomp1 = malloc(numElms2 + 1);
    status = CDFlib(SELECT_, ATTR_NAME_, "COMPONENT_0",
                             zENTRY_NAME_, varName,
                    GET_, zENTRY_DATA_, tcomp0,
                    SELECT_, ATTR_NAME_, "COMPONENT_1",
                             zENTRY_NAME_, varName,
                    GET_, zENTRY_DATA_, tcomp1,
                    NULL_);
    if (status != CDF_OK) {
      return status;
    } 
    tcomp0[(int)numElms1] = (char) 0;
    tcomp1[(int)numElms2] = (char) 0;
    strcpy (component0, tcomp0);
    strcpy (component1, tcomp1);
    free (tcomp0);
    free (tcomp1);
    tcomp0 = NULL;
    tcomp1 = NULL;
...
char *tcomp0, *tcomp1;
  ...
  status = CDFlib(SELECT_, ATTR_NAME_, "COMPONENT_0",
                  ...
                  GET_, BOO(zVar,zENTRY_NUMELEMS_,rENTRY_NUMELEMS_), &numElms1,
                        BOO(zVar,zENTRY_DATATYPE_,rENTRY_DATATYPE_), &dataType,
                  SELECT_, ATTR_NAME_, "COMPONENT_1",
                           BOO(zVar,zENTRY_NAME_,rENTRY_NAME_), qaName,
                  GET_, BOO(zVar,zENTRY_NUMELEMS_,rENTRY_NUMELEMS_), &numElms2,
                  NULL_);
  if (status != CDF_OK) {
    return status;
  }
