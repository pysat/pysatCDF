...
void EncodeString (nBytes, iString, oString, minWidth, maxWidth)
long nBytes;            /* Number of bytes (elements). */
...
{
  char delim = PickDelimiter (iString, (size_t) nBytes); int i;
  long numChars, newBytes; 
  /****************************************************************************
  * Encode string.
  ****************************************************************************/
  newBytes = MINIMUM(nBytes, (long) strlen(iString));
  numChars = (long) UTF8StrLength((char *)iString, newBytes);
  if ((numChars + 2) > BOO(maxWidth == 0,(int)(numChars + 2),maxWidth)) {
    /**************************************************************************
    ...
    **************************************************************************/
    if (maxWidth < 6) {
      ...
      oString[0] = delim;
      memcpy (oString+1, iString, (size_t)  maxWidth - 5);
      oString[maxWidth-4] = '.';
      oString[maxWidth-3] = '.';
      oString[maxWidth-2] = '.';
      oString[maxWidth-1] = delim;
      oString[maxWidth]   = NUL;
    }
  }
  else {
    /**************************************************************************
    ...
    **************************************************************************/
    oString[0] = delim;
    memcpy (oString+1, iString, (size_t) newBytes);
    oString[newBytes+1] = delim;
    oString[newBytes+2] = NUL;
  }
  /****************************************************************************
  * Blank pad to `minWidth' if necessary.
  ****************************************************************************/
  if (minWidth != 0) {
    int n = UTF8StrLength((char *)oString, newBytes+2);
    if (minWidth > 0) {         /* Right justify. */
      if (n < minWidth) {
	int i, pad = minWidth - n;
...
#define SEP_CHAR ';'
  ...
  envPtr = getenv ("PATH");
  if (envPtr != NULL) {
    char *path = (char *) cdf_AllocateMemory ((size_t)strlen(envPtr) + 1,
					      FatalError);
    char *p1 = path;
...
int done;
  ...
  ****************************************************************************/
  if (nFiles > 0) {
    ...
    **************************************************************************/
    *dirS = (char **) cdf_AllocateMemory ((size_t)(nFiles*sizeof(char *)) +
					  strlen(dir)+1, FatalError);
    *nameS = (char **) cdf_AllocateMemory ((size_t)(nFiles*sizeof(char *)) +
					   tNameChars, FatalError);
...
for (i = 0; pattern[i] != NUL; i++)
   switch (pattern[i]) {
	 ...
	 types[nTypes] = STR;
	 strings[nTypes] =
		 (char *) cdf_AllocateMemory ((size_t)DU_MAX_NAME_LEN + 1,
					      FatalError);
