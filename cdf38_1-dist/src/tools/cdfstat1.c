...
int virtual;
{
  ...
  ****************************************************************************/
  ...
  AllocateBuffers (Var->varMaxRec + 1, Var->numDims, dimSizes, &groups, 0, 1,
		   ...
		   FatalError);
  if (STRINGdataType(Var->dataTypeV)) extra = 1;
  else extra = 0;
  Var->min = cdf_AllocateMemory ((size_t) Var->nValueBytes+extra,
			     FatalError);
  Var->max = cdf_AllocateMemory ((size_t) Var->nValueBytes+extra,
			     FatalError);
  Var->last = cdf_AllocateMemory ((size_t) Var->nValueBytes+extra,
			      FatalError);
  if (Var->rangeCheckVar) {
    Var->minINrange = cdf_AllocateMemory ((size_t) Var->nValueBytes+extra,
				      FatalError);
    Var->maxINrange = cdf_AllocateMemory ((size_t) Var->nValueBytes+extra,
				      FatalError);
  }
  /****************************************************************************
  * Read each value...
  ****************************************************************************/
  ...
  InitHyperParms (&hyperSt, &groups, Var->numDims, &nHypers, &nValues);
  for (hyperN = 0; hyperN < nHypers; hyperN++) {
     status = CDFlib (SELECT_, BOO(Var->Z,zVAR_RECNUMBER_,
			       ...
			       BOO(Var->Z,zVAR_RECCOUNT_,
					  rVARs_RECCOUNT_), hyperSt.recCount,
	...
	if (Var->checkMonotonicVar) Monotonic (Var);
     ...
     IncrHyperParms (&hyperSt, &groups, Var->numDims, ROWmajor(majority),
		     &nValues);
     CHECKforABORTso
  }
  /****************************************************************************
...
struct VarStructST *Var;
{
  ...
  if (use) {
    ...
    if (STRINGdataType(Var->dataTypeV))
      *(((char *)Var->min)+(int)Var->numElemsV) = '\0';
    ASSIGNx (Var->max, Var->value, Var->dataTypeV, Var->numElemsV);
    if (STRINGdataType(Var->dataTypeV))
      *(((char *)Var->max)+(int)Var->numElemsV) = '\0';
    if (Var->rangeCheckVar) {
	  ...
	  ASSIGNx (Var->minINrange, Var->value, Var->dataTypeV,
		   Var->numElemsV);
	  if (STRINGdataType(Var->dataTypeV))
	    *(((char *)Var->minINrange)+(int)Var->numElemsV) = '\0';
	  ASSIGNx (Var->maxINrange, Var->value, Var->dataTypeV,
		   Var->numElemsV);
	  if (STRINGdataType(Var->dataTypeV))
	    *(((char *)Var->maxINrange)+(int)Var->numElemsV) = '\0';
        }
    }
...
struct VarStructST *Var;
{
  ...
  if (use) {
    if (!Var->monoInited) {
      ...
      if (STRINGdataType(Var->dataTypeV))
        *(((char *)Var->last)+(int)Var->numElemsV) = '\0'; 
      Var->monoState = _Init;
      Var->monoInited = TRUE;
    }
    else {
