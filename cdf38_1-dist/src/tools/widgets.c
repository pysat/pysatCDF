     ...
     case READiw:
	  ...
	  if (IW->nItems > 0) {
	    ...
	    set_cursor_abs (IW->wid, IW->iRowN,
			    curCol + UTF8TotalWidth(anItem) + 1);
	    free (anItem);
	    anItem = NULL;
	  }
   ...
   switch (op) {
     ...
     *************************************************************************/
	 ...
	 put_chars (PW->wid, " ", 1, PW->vRow, PW->mRcol, FALSE, NORMAL);
       ...
       PW->curLen = UTF8StrLength (PW->value, (int) strlen(PW->value));
       PW->curWidthLen = UTF8TotalWidth (PW->value);
       if (PW->curCharWidth < PW->NvCols) {
	 if (PW->curWidthLen > PW->NvCols) {
	   int lastCharNum = UTF8LastCharWithinWidth (PW->value, PW->NvCols);
	   ...
	   put_chars (PW->wid, PW->value, PW->curLen, PW->vRow, PW->vLcol,
		      FALSE, NORMAL);
	 PW->curCol = PW->vLcol + UTF8FirstCharsWidth(PW->value, PW->curChar);
       } else {
	 put_chars (PW->wid, "<", 1, PW->vRow, PW->mLcol, FALSE, NORMAL);
     ...
     *************************************************************************/
	  ...
	  ********************************************************************/
	  if (PW->key == KB_RIGHTARROW) {
		...
		if (PW->curChar == PW->curLen) {
		  put_chars (PW->wid, UTF8CharAtString(PW->value,leftColChar),
		...
		} else
{
	  ...
	  ********************************************************************/
	  if (PW->key == KB_DELETE) {
	    ...
	    char blanks[5];
            iy = (int) strlen(PW->value);
	    ix = UTF8StrLength (PW->value, iy);
	    len =  UTF8CharAtLength (PW->value, PW->curChar-1);
	    width =  UTF8CharAtWidth (PW->value, PW->curChar-1);
	    memset (blanks, ' ', (size_t)width);
	    blanks[width] = '\0';
	    begin_pasteboard_update ();
	    if (PW->curCol == PW->vLcol) {
	      if (PW->curChar > 0) {
		...
		PW->curWidthLen = UTF8TotalWidth (PW->value);
		if (PW->curChar == 0) 
		  put_chars (PW->wid, blanks, width, PW->vRow, PW->mLcol,
			     FALSE, NORMAL);
	      } else
		ring_bell ();
	    } else {
		...
		PW->curWidthLen = UTF8TotalWidth (PW->value);
		put_chars (PW->wid, UTF8CharAtString(PW->value, PW->curChar),
			   colsToEnd, PW->vRow, PW->curCol, FALSE, NORMAL);
		offRight--;
		if (offRight == 0) 
		  put_chars (PW->wid, blanks, width, PW->vRow, PW->mRcol,
			     FALSE, NORMAL);
	      } else {
		int w2;
		if (charsToEnd > 0) {
		  ...
		  PW->curWidthLen = UTF8TotalWidth (PW->value);
		  put_chars (PW->wid, UTF8CharAtString(PW->value, PW->curChar),
			     charsToEnd, PW->vRow, PW->curCol, FALSE, NORMAL);
		  w2 = UTF8TotalWidth (UTF8CharAtString(PW->value,
				       PW->curChar));
		  put_chars (PW->wid, blanks, width, PW->vRow,
			     PW->curCol + w2, FALSE, NORMAL);
		} else {
		  PW->curChar--;
	  ...
	  ********************************************************************/
	  if (PW->key == PW->EOLchar) {
	    ...
	    if (rightColChar < PW->curLen) {
	      put_chars (PW->wid, "<", 1, PW->vRow, PW->mLcol, FALSE, NORMAL);
	      put_chars (PW->wid, UTF8CharAtString(PW->value, PW->curLen -
							      PW->NvCols + 1),
			 PW->NvCols - 1, PW->vRow, PW->vLcol, FALSE, NORMAL);
	      put_chars (PW->wid, " ", 1, PW->vRow, PW->vRcol, FALSE, NORMAL);
	      put_chars (PW->wid, " ", 1, PW->vRow, PW->mRcol, FALSE, NORMAL);
	    }
	    PW->curCol = MinInt (PW->vLcol + PW->curWidthLen, PW->vRcol);
	  ...
	  ********************************************************************/
	  ...
	  if (ic > 0) {
		...
		PW->curLen = PW->curLen + (int) UTF8CharWidth(gotKey);
		PW->curChar++;
		if (colsToEnd > 1) {
		  put_chars (PW->wid, UTF8CharAtString(PW->value,
						       PW->curChar-1),
		...
		if (PW->insertMode) {
		  ...
		  ************************************************************/
		  char *ptr = UTF8CharAtString (PW->value, PW->curChar);
		  int len =  UTF8CharLength (gotKey);
		  memmove (ptr+len, ptr, strlen(ptr) + 1);
		  memmove (ptr, (char *) gotKey, len);
		  PW->curLen = PW->curLen + UTF8CharWidth(gotKey);;
		  if (colsToEnd > 1) {
		    /**********************************************************
		  ...
		  ************************************************************/
		  ...
		  int  iy = UTF8CharAtLength(PW->value,PW->curChar);
		  memmove (p+ic, p+iy, strlen(p+iy) + 1);
		    ...
		    **********************************************************/
			...
			put_chars (PW->wid, " ", 1, PW->vRow, PW->mRcol, FALSE,
		      ...
		      ********************************************************/
		      ...
		      leftColChar = PW->curChar - PW->NvCols + 1;
		      put_chars (PW->wid,
				 UTF8CharAtString(PW->value, leftColChar),
				 PW->NvCols - 1, PW->vRow, PW->vLcol, FALSE,
				 NORMAL);
		      put_chars (PW->wid, " ", 1, PW->vRow, PW->vRcol, FALSE,
				 NORMAL);
   ...
   switch (op) {
     ...
     *************************************************************************/
	 ...
	 DrawSection (EW->wid, EW->hRowT, EW->hRowB, 0,
       ...
       * rows.
       ***********************************************************************/
       EW->nChars = UTF8StrLength (EW->eText, (int) strlen(EW->eText));
       EW->xChars = EW->nChars;
     ...
     *************************************************************************/
	  ...
	  ********************************************************************/
	  ...
	  if (ic > 0) {
		...
		memmove (p1+ic, p1, strlen(p1+length) + 1); 
		memmove (p1, (char *) gotKey, ic);
	      }
	      DrawEditSection (EW);
...
struct EditWindowStruct *EW;
{
  ...
  for (;;) {
     ...
     if (nextNL == NULL) {
       length = UTF8StrLength (charPtr, (int) strlen(charPtr));
       width = UTF8TotalWidth (charPtr);
       if (length > 0) {
     ...
     tmp2[length] = (char) '\0';
     length2 = UTF8StrLength (tmp2, (int) strlen(tmp2));
     put_chars (EW->wid, tmp2, MINIMUM(length2,EW->nColS), rowN, 1, FALSE,
		NORMAL);
     width = UTF8TotalWidth (tmp2);
     ...
     *************************************************************************/
	  ...
	  ********************************************************************/
	  if (FW->key == FW_LEFT_CHAR || FW->key == FW_LEFT_CHARx) {
		...
		len = UTF8StrLength (ptr, (int) strlen(ptr));
	  ...
	  ********************************************************************/
	  if (FW->key == FW_RIGHT_CHAR || FW->key == FW_RIGHT_CHARx) {
	    ...
	    fieldLen = UTF8StrLength(pFieldN, (int) strlen(pFieldN));
	  ...
	  ********************************************************************/
	  if (FW->key == KB_DELETE) {
	    ...
	    if (FW->charN > 0) {
	      int ix;
	      ix = UTF8StrLength (ptr0, (int) strlen(ptr0));
	      if (FW->charN == FW->leftCharN) {
		char *ptr = UTF8CharAtString (ptr0, FW->charN);
		int len =  UTF8CharAtLength (ptr0, FW->charN-1);
		memmove (ptr-len, ptr, strlen(ptr) + 1);
		FW->charN--;
		FW->leftCharN--;
	      }
		...
		int len;
		char *ptr = UTF8CharAtString (ptr0, FW->charN);
		len =  UTF8CharAtLength (ptr0, FW->charN-1);
		memmove (ptr-len, ptr, strlen(ptr) + 1);
		FW->charN--;
		DrawField (FW);
	      }
	  ...
	  ********************************************************************/
	  ...
	  if (ic > 0) {
	    if (FW->insert) {
	      char *ptr0 = FW->fields[FW->fieldN];
	      int len = UTF8StrLength (ptr0, (int) strlen(ptr0)); 
		...
		ring_bell ();
	      ...
	      if (FW->charN < FW->fMaxs[FW->fieldN]) {
	        char *ptr0 = FW->fields[FW->fieldN];
		int len2;
		int len = UTF8StrLength(ptr0, (int) strlen(ptr0)); /* char # */
...
struct FieldWindowStruct *FW; /* Pointer to FieldWindow structure. */
{
  ...
  for (rowN = FW->fRowT, lineN = FWtopRowLineN(FW);
	...
	if (FW->fLineNs[fieldN] == lineN) {
	  ...
	  len = UTF8StrLength (aField, (int) strlen(aField));
	  fColN = FW->fCols[fieldN];
	  for (i = 0; i < FW->fLens[fieldN]; i++) {
	     if (i < (int) len) {
	       charLen = UTF8CharAtLength (aField, i);
     ...
     len = UTF8StrLength (line, (int) strlen(line));
     width = UTF8TotalWidth (line);
			   ...
			   line is displayed in column one [1]). */
{
  ...
  for (rowN = rowT, lineN = topRowLineN;
     ...
     len = UTF8StrLength (lineS[lineN], (int) strlen(lineS[lineN]));
     put_chars (wid, lineS[lineN], MINIMUM(nColS,len), rowN, 1,
		FALSE, NORMAL);
     width = UTF8TotalWidth (lineS[lineN]);
     if (width < nColS)
